...
const createUser = async (req, res) => {
  try {
    const { name, email, industry, role } = req.body;
  ...
  `,
    ...
    sendEmails(email, "New Account", emailData.html, emailData.attachments);
...
const getAllActivities = async (req, res) => {
  try {
    ...
    const [countResult, activities] = await Promise.allSettled([
      ...
      ActivityModel.find({ title: { $regex: search, $options: "i" } })
        .populate("IndustryID")
        .populate("RoleID")
        .sort({ [sortField]: sort })
        .skip(offset)
...
const getAllIndustries = async (req, res) => {
  try {
    ...
    const [countResult, industries] = await Promise.allSettled([
      ...
      IndustryModel.find({ title: { $regex: search, $options: "i" } })
        .sort({ [sortField]: sort })
        .skip(offset)
        .limit(limit),
    ]);
...
const getIndustriesByfireBase = async (req, res) => {
  try {
    const response = await fetch(
      "https://oz-api-prod.azurewebsites.net/api/industryNames/list"
    );
    if (!response.ok) {
    ...
    for (const industry of industriesbyfireBase.data) {
      const query = "SELECT * FROM industries WHERE title = $1 LIMIT 1";
      const existingIndustry = await pool.query(query, [industry.title]);
    ...
    const industryTitles = industriesbyfireBase.data.map(
      (industry) => industry.title
    );

    const query = `SELECT * FROM industries WHERE title = ANY($1) ORDER BY title ASC;`;
    const result = await pool.query(query, [industryTitles]);
    const matchingIndustries = result.rows;
    console.log("matchingIndustriestt", result.rows);
...
const getAllRoles = async (req, res) => {
  try {
    ...
    const [countResult, roles] = await Promise.allSettled([
      ...
      RoleModel.find({ title: { $regex: search, $options: "i" } })
        .sort({ [sortField]: sort })
        .populate("IndustryID")
        .skip(offset)
        .limit(limit),
...
export const sendNotifications = async (req, reply) => {
  try {
    ...
    if (type === "All") {
      const fetchUserTokens = await DeviceModel.aggregate([
        {
          $lookup: {
            from: "auths",
            localField: "user",
      ...
      const fetchAllToken =
        fetchUserTokens.length > 0 ? fetchUserTokens[0].total : 0;
      ...
      for (let i = 0; i < divideResult; i++) {
        ...
        const fetchAllTokenWithSkip = await DeviceModel.aggregate([
          {
            $lookup: {
              from: "auths",
              localField: "userId",
        ...
        const arrayOfDevice = fetchAllTokenWithSkip.map(
          (obj) => obj.deviceToken
        );
        const message = {
      ...
      if (userIds.length == 0) {
        return reply.send(
          CustomError.createError("User IDs length must be greater than 0", 400)
        );
      }
      ...
      if (userIds.length > 100) {
        return reply.send(
          CustomError.createError(
            "User IDs length must be less than or equal to 100",
            400
          )
        );
      }
      const fetchAllToken = await DeviceModel.aggregate([
        {
          $lookup: {
            from: "auths",
            localField: "userId",
...

        {
          $match: {
            ...
            "user.notificationOn": true, // Condition for notifications
            userId: {
              $in: userIds.map((el) => new mongoose.Types.ObjectId(el)),
            },
          },
        },
      ...
      fcm.send(message, async function (err, response) {
        ...
        const createNotification = await NotificationsModel.insertMany(
          Notifications
        );
        console.log("Successfully sent with response:", response);
      });
...
const getUserCases = async (req, res) => {
  try {
    ...
    const [countResult, userCases] = await Promise.allSettled([
      ...
      CaseNumberModel.find({ caseNumber: { $in: caseIDS } })
        .sort({ [sortField]: sort })
        .skip(offset)
        .limit(limit),
    ]);
...
const getCaseDetails = async (req, res, next) => {
  try {
    ...
    const userDetails = await PreAuditModel.findOne({ caseNumber: caseNumber })
      .populate("user", "_id email phone name notificationOn userType")
      .select("user");
    const preAudits = await PreAuditModel.find({
      caseNumber: caseNumber,
    }).populate("ActivityID");
    const audits = await AuditModel.find({ caseNumber: caseNumber }).populate({
      path: "ActivityID",
    });
    const nonValueActivities = await NonValueActivtyModel.find({
      caseNumber: caseNumber,
